...
export const AS_OF = '2026-09-10T23:59:59Z';
...
export type Token = (typeof assets)[number]['symbol'];
...
const fixedAddress = (n: number) => '0x' + n.toString(16).padStart(40, '0');
...
export function buildWalletData(address: string, days: number) {
  const normalizedAddress = address.toLowerCase();
  const exampleProfile = exampleWallets.find(
  ...
  const lifecycleMode =
    exampleProfile === 'Dormant or new wallet' ? 0 : seed % 11;
  const behaviorMode =
    exampleProfile === 'Active trader'
      ? 0
  ...
  const opening = Object.fromEntries(
    assets.map((a) => [a.symbol, round(a.opening * factor)]),
  ) as Record<Token, number>;
  const balances = { ...opening };
  const all: Transaction[] = [];
  for (let age = 179; age >= 0; age--) {
    if (lifecycleMode === 0 && age > 20) continue;
    if (lifecycleMode === 1 && age < 45) continue;
    const count =
      exampleProfile === 'Long-term holder'
        ? age % 12 === seed % 12
          ? 1
          : 0
        : (age * 17 + seed) % 9 < 2
    ...
    for (let i = 0; i < count; i++) {
      ...
      let kind = baseKind;
      if (behaviorMode === 0 && baseKind >= 10) kind = baseKind % 4;
      if (behaviorMode === 1 && baseKind >= 8) kind = 4 + (baseKind % 4);
      if (behaviorMode === 2 && baseKind >= 10) kind = 8 + (baseKind % 2);
      if (behaviorMode === 3 && baseKind >= 8) kind = 6 + (baseKind % 2);
      ...
      const hash =
        ...
        Array.from({ length: 8 }, (_, j) =>
          ((Math.imul(seed + n + 1, 2654435761 + j * 97) ^ (j * 123457)) >>> 0)
            .toString(16)
            .padStart(8, '0'),
        ).join('');
      const time = new Date(
        Date.UTC(2026, 8, 10 - age, 23 - Math.floor(i / 2), (i * 17) % 60),
      ).toISOString();
      const protocol =
        kind < 4
          ? 'Uniswap'
      ...
      const counterparty =
        protocol === 'No protocol' ? 'Other wallets' : protocol;
      const movements: Movement[] = [];
      function move(token: Token, direction: Direction, amount: number) {
        const tokenPrice = assets.find((a) => a.symbol === token)!.price;
        const units = round(amount);
        balances[token] = round(
          balances[token] + (direction === 'in' ? units : -units),
        );
        movements.push({
      ...
      if (kind < 4) {
        type = 'Swap';
        const token: Token = kind < 2 ? 'ETH' : 'WBTC';
        const price = assets.find((a) => a.symbol === token)!.price;
        if (kind % 2 === 0) {
          move('USDC', 'out', value);
  ...
  const orderedHistory = [...all].sort((a, b) =>
  ...
  const walletAgeDays = firstActivity
    ? Math.floor((Date.parse(AS_OF) - Date.parse(firstActivity)) / DAY) + 1
    : 0;
  const daysSinceLastActivity = lastActivity
    ? Math.floor((Date.parse(AS_OF) - Date.parse(lastActivity)) / DAY)
    : null;
  ...
  const protocols = protocolDefinitions.map((p) => {
  ...
  return {
    ...
    counterparties: new Set(
      movements.map((m) => (m.direction === 'in' ? m.from : m.to)),
    ).size,
    firstActivity,
    lastActivity,
    walletAgeDays,
...
export type WalletData = ReturnType<typeof buildWalletData>;
